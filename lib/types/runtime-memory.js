...
export const RUNTIME_MEMORY_LIMITS = { memory: 10 * 1024, user: 4 * 1024 };
...
function normalizeContent(value, field) {
    ...
    if (content.includes('§'))
        throw new Error(`${field} must not contain the reserved § entry delimiter`);
    const bytes = Buffer.byteLength(content, 'utf8');
    if (bytes > MAX_ENTRY_BYTES)
        throw new Error(`${field} is too large (${bytes} bytes; max ${MAX_ENTRY_BYTES})`);
    return content;
}
function parseEntry(value) {
    if (!isRecord(value) || typeof value.content !== 'string' || !isTarget(value.target) || !isImportance(value.importance))
        return undefined;
...
export class RuntimeMemoryController {
    ...
    deliveredRevisions = new WeakMap();
    deliveredGlobalRevision;
    constructor(runner, now = () => new Date()) {
        this.now = now;
    ...
    contextText(scope, force = false) {
        ...
        if (!force && previousRevision === snapshot.revision)
            return '';
        if (scope === undefined)
            this.deliveredGlobalRevision = snapshot.revision;
        else
            this.deliveredRevisions.set(scope, snapshot.revision);
...
IMPORTANT: USER.md and MEMORY.md above are always relevant when applicable. Follow the current user's request first, use mnemon_runtime_memory proactively only when the write criteria are met, and otherwise continue without a memory mutation.`;
    ...
    compactTarget(expectedRevision, target, compacted, maxBytes = RUNTIME_MEMORY_LIMITS[target]) {
        const operation = this.queue.then(() => this.withLock(() => {
            const file = this.readSource();
            if (revision(file) !== expectedRevision)
                throw new RuntimeMemoryConflictError();
            if (!Number.isInteger(maxBytes) || maxBytes < 0 || maxBytes > RUNTIME_MEMORY_LIMITS[target])
            ...
            const replacements = compacted.map((entry) => {
                ...
                if (!isImportance(entry.importance))
                    throw new Error('compacted importance must be critical, normal, or low');
                if (seen.has(content))
                    throw new Error('compacted runtime memory contains duplicate entries');
                seen.add(content);
            ...
            const ranked = replacements.map((entry, index) => ({ entry, index })).sort((left, right) => (priority[left.entry.importance] - priority[right.entry.importance] || left.index - right.index));
            const selected = new Set();
            const packed = [];
            for (const candidate of ranked) {
                if (byteCount([...packed, candidate.entry], target) > maxBytes)
                    continue;
            ...
            if (used > limit)
                throw new RuntimeMemoryCapacityError(target, byteCount(file.entries, target), used, limit);
    ...
    mutateLocked(request) {
        ...
        if (!['add', 'replace', 'remove'].includes(request.action))
            throw new Error('action must be add, replace, or remove');
        if (request.importance !== undefined && !isImportance(request.importance))
            throw new Error('importance must be critical, normal, or low');
        const file = this.readSource();
        ...
        if (request.action === 'add') {
            ...
            if (matches.length === 0)
                throw new Error(`No ${request.target} entry contains ${JSON.stringify(oldText)}.`);
            if (matches.length > 1)
                throw new Error(`Multiple ${request.target} entries contain ${JSON.stringify(oldText)}; use a unique substring.`);
        ...
        if (used > limit)
            throw new RuntimeMemoryCapacityError(request.target, byteCount(before, request.target), used, limit);
        this.persist({ version: RUNTIME_MEMORY_VERSION, entries });
    ...
    readSource() {
        if (!existsSync(this.sourcePath))
            return { version: RUNTIME_MEMORY_VERSION, entries: [] };
        let parsed;
        try {
        ...
        return { version: RUNTIME_MEMORY_VERSION, entries: entries };
    }
    persist(file) {
        mkdirSync(this.directory, { recursive: true, mode: 0o700 });
        ...
        while (descriptor === undefined) {
            try {
                ...
                if (Date.now() - started >= LOCK_TIMEOUT_MS)
                    throw new Error('timed out waiting for the runtime memory controller lock');
