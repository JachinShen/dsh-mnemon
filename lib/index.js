...
const Config = z.object({
	...
	runtimeMemoryMode: z.union([
		"session-revision",
		"every-turn",
		"off"
	]).default("session-revision"),
	recallMode: z.union(["guided", "off"]).default("guided"),
	lifecycleCueMode: z.union([
		"session",
		"every-turn",
		"off"
	]).default("session"),
...
function registerRuntimeMemoryContext(ctx, runtimeMemory, mode) {
	if (mode === "off") return;
	systemPrompt(ctx)?.section?.({
...
var RuntimeMemoryController = class {
	...
	contextText(scope, force = false) {
		...
		const previousRevision = scope === void 0 ? this.deliveredGlobalRevision : this.deliveredRevisions.get(scope);
...
function guidedReminder(config) {
	if (config.lifecycleCueMode === "off") return void 0;
